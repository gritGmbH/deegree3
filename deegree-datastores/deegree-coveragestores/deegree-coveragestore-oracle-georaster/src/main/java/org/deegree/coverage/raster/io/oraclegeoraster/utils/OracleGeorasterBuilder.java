...
public class OracleGeorasterBuilder implements ResourceBuilder<Coverage> {
    ...
    public OracleGeorasterBuilder( OracleGeorasterConfig config, ResourceMetadata<Coverage> metadata, Workspace workspace ) {
        this.config = config;
	...
	public Coverage build()  {
		try {
            OracleGeorasterReader rdr = new OracleGeorasterReader( workspace, config );
            
            AbstractRaster rasterLvl0 = rdr.getRaster();
	        ...
	        throw new ResourceInitException( "Could not build Oracle GeoRaster Reader.", e );
	    }
	}
}